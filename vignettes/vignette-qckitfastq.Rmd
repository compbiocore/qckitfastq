...
Some functions in this package are based on `seqTools` given that their functions are relatively fast. Thus, we will first process the file through seqTools and then let qckitfastq functions proceed with the analysis. 
...
The *basic_statistics* function calculates the quality score per sequence statistics including mean, median and quantiles. We can use the result to create a quality score distribution per position plot to present the overall dispersion and concentration of quality score per position of the FASTQ file.
